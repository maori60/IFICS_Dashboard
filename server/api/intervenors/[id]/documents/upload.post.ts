...
export default defineEventHandler(async (event) => {
  ...
  const document = await prisma.intervenorDocument.create({
    data: { intervenorId, type, title, status: 'PENDING', version: (previous?.version || 0) + 1, ...stored },
    select: { id: true, type: true, status: true, title: true, originalName: true, fileSize: true, sha256: true, version: true, createdAt: true },
  })

  await writeAuditLog(event, context, { action: 'INTERVENOR_DOCUMENT_UPLOADED', entityType: 'IntervenorDocument', entityId: document.id, metadata: { intervenorId, type, sha256: document.sha256 } })
