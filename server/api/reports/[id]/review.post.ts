...
export default defineEventHandler(async (event) => {
  ...
  if (report.intervenor?.userId) await notifyUser(report.intervenor.userId, status === 'VALIDATED' ? 'SUCCESS' : 'WARNING', 'Bilan examiné', status === 'VALIDATED' ? 'Votre bilan a été validé.' : `Votre bilan a été refusé : ${refusalReason}`, `/reports/${id}`)
