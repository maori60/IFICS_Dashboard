...
import { notifyRoleCodes } from '../../../utils/notifications'
...
export default defineEventHandler(async (event) => {
  ...
  const report = await prisma.projectReport.findFirst({ where: { id, project: { associationId: context.associationId, archivedAt: null } }, select: { id: true, projectId: true, intervenorId: true, currentVersion: true, status: true, project: { select: { title: true } } } })
  ...
  await notifyRoleCodes(context.associationId, ['ADMIN', 'MANAGER'], 'ACTION', 'Bilan à valider', `Un bilan du projet « ${report.project.title} » attend une validation.`, `/reports/${id}`)
